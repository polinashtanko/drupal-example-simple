Docker Compose Drupal 9 Elastic - php7.4, nginx, mariadb, elasticsearch
=======================================================================
...
docker run --rm --net drupal9-example-elastic_default jwilder/dockerize dockerize -wait tcp://mariadb:3306 -timeout 1m
```
...
docker ps --filter label=com.docker.compose.project=drupal9-example-elastic | grep Up | grep drupal9-example-elastic_nginx_1
docker ps --filter label=com.docker.compose.project=drupal9-example-elastic | grep Up | grep drupal9-example-elastic_mariadb_1
docker ps --filter label=com.docker.compose.project=drupal9-example-elastic | grep Up | grep drupal9-example-elastic_php_1
docker ps --filter label=com.docker.compose.project=drupal9-example-elastic | grep Up | grep drupal9-example-elastic_cli_1

# Should ssh against the cli container by default
docker-compose exec -T cli bash -c "env | grep LAGOON=" | grep cli-drupal

# Should have the correct environment set
docker-compose exec -T cli bash -c "env" | grep LAGOON_ROUTE | grep drupal9-example-elastic.docker.amazee.io
